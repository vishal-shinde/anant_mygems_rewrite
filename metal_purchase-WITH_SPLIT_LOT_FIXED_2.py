...
LENGTH_OPTIONS = ["0", "5.5", "6", "6.5", "7", "7.5", "8", "12", "15.5", "16", "16.5", "17", "18", "20", "22", "24", "26", "28", "30", "32"]
    ...
    def _open_adjustment_dialog(self, total_amount):
        ...
        def save():
            ...
            if adj_amt > 0:
                ...
                # FIX (bug #22, carried over): validate the lot exists —
                # either already in this bill, or already in the database.
                lot_in_this_bill = any(
                    str(self.tree.item(i)["values"][0]).strip().upper() == new_lot
                    for i in self.tree.get_children()
                )
                lot_in_db = False
                if not lot_in_this_bill:
                    try:
                        self.cur.execute(
                            f"SELECT 1 FROM {self.cfg['jewelry_table']} WHERE UPPER(lot_no) = %s LIMIT 1",
                            (new_lot,)
                        )
                        lot_in_db = self.cur.fetchone() is not None
                    except Exception as e:
                        messagebox.showerror("Error", f"Lot check failed:\n{e}"); return
                if not (lot_in_this_bill or lot_in_db):
                    messagebox.showerror(
                        "Invalid Lot",
                        f"'{new_lot}' isn't in this bill and doesn't exist in the database.\n"
                        "Add it to the bill first, or check the lot number for typos."
                    )
                    return
        ...
        try:
            ...
            if is_buyback:
                cur.execute("""SELECT customer_id FROM customers 
                               WHERE UPPER(name)=%s LIMIT 1""", (sup,))
                res = cur.fetchone()
            ...
            for r in items:
                ...
                if entry_type == "SPLIT_LOT":
                    ...
                    cur.execute(f"""INSERT INTO {self.cfg['jewelry_table']} (lot_no, purity, purchase_no, jewelry_type, supplier_name, description, status, dimension, item_code) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""", 
                                (lot, pur, pno, jtype, sup, desc, 'MFG', dim, code))
                    cur.execute(f"""INSERT INTO {self.cfg['account_table']} (txn_date, purchase_no, lot_no, per_gram, weight, amount, txn_nature, jewelry_code) VALUES (%s, %s, %s, %s, %s, %s, 'DR', %s)""", 
                                (txn_date, pno, lot, pg, wt, amt, code))
            ...
            # were always fully settled. That's no longer true — a partial            # adjustment can still leave a real ADVANCE/PENDING balance on
